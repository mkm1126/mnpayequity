...
import { ArrowLeft, Search, AlertTriangle, CheckSquare, Square } from 'lucide-react';
import { supabase, type Jurisdiction, type Contact } from '../lib/supabase';
import { EmailComposer } from './EmailComposer';

type FailToReportJurisdictionListProps = {
  selectedYear: number;
  onBack: () => void;
};
...
export function FailToReportJurisdictionList({
  selectedYear,
  onBack,
}: FailToReportJurisdictionListProps) {
  ...
  async function loadJurisdictions() {
    try {
      ...
      const { data: reports, error: reportsError } = await supabase
        .from('reports')
        .select('jurisdiction_id, case_status')
        .eq('report_year', selectedYear);

      if (reportsError) throw reportsError;

      const jurisdictionsWithSubmittedReports = new Set(
        reports?.filter((r) => r.case_status === 'Submitted').map((r) => r.jurisdiction_id) || []
      );

      const jurisdictionsWithoutReports = (jurisdictionsData || []).filter(
        (j) => !jurisdictionsWithSubmittedReports.has(j.id)
      );

      const jurisdictionsWithContacts: JurisdictionWithContact[] = [];

      for (const jurisdiction of jurisdictionsWithoutReports) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Fail to Report Jurisdiction List</h1>
          <p className="text-sm text-gray-600">
            The following {jurisdictions.length} jurisdiction{jurisdictions.length !== 1 ? 's have' : ' has'} not submitted reports for year {selectedYear}
          </p>
        </div>
      </div>
      ...
      <div className="bg-red-50 border-l-4 border-red-400 p-4 rounded-r-lg mb-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-red-800">Official Compliance Notice</h3>
            <p className="text-sm text-red-700 mt-1">
              An official Fail To Report letter will be sent with your email. Please fill out the subject and message body for your email notice.
              You can unselect a jurisdiction by unchecking the box in front of the ID.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm">
              {jurisdictions.length === 0
                ? 'All jurisdictions have submitted their reports!'
        ...
        <div className="mt-6 flex items-center justify-between pt-4 border-t border-gray-200">
          ...
          >
            ...
            Compose Notice ({selectedJurisdictions.size})
          </button>
        </div>
      </div>
    </div>
